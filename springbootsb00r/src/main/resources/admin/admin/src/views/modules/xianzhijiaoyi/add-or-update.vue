    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="物品状态" prop="wupinzhuangtai">
          <el-select :disabled="ro.wupinzhuangtai" v-model="ruleForm.wupinzhuangtai" placeholder="请选择物品状态">
            <el-option
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.shoujihaoma!=''&&json.shoujihaoma){
          ...
          this.$message.error(data.msg);
            ...
            this.wupinzhuangtaiOptions = "未交易,已交易".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xianzhijiaoyi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.wupinxiangqing = this.ruleForm.wupinxiangqing.replace(reg,'../../../springbootsb00r/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xianzhijiaoyi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xianzhijiaoyiCrossAddOrUpdateFlag = false;
						 this.parent.search();
